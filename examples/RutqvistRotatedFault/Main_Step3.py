...
if __name__ == '__main__':
    ...
    print(" ")
    print(timer.ctime())
    print_title("Final mesh generation process started")
    ...
    project_batch_file_name = str(json_data['project_name'])+'_step3'+'.bch'
    ...
    # Generate the final mesh (volumes and surfaces) using the final mesh size
    generate_mesh_time_start = timer.perf_counter()
    tcl_proc = 'ExxonPoromechanicsApplication::FinalMesh'
    os.chdir(gid_path)
    ...
    os.chdir(project_path)
    print("Time to generate the final mesh:       %.3f [s]" % (timer.perf_counter() - generate_mesh_time_start))
    ...
    # Create output step3 folder
    output_files_directory_path = os.path.join(str(output_files_directory_path),'step3')
    output_files_directory_path = output_files_directory_path.replace(os.sep, '/')
    ...
    # NOTE: Uncomment the following lines if you want to keep a copy of the GiD problem
    # src = os.path.join(temporary_files_directory_path, str(json_data['project_name']) + '.gid')
    # dst = os.path.join(project_path + '/output', str(json_data['project_name']) + '.gid')
    # shutil.move(src, dst)

    # Erase the temporary folder
    shutil.rmtree(temporary_files_directory_path)
    ...
    print_title("Final mesh generation process finished")